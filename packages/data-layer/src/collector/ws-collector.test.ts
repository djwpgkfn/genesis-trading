...
class FakeTransport implements WsTransport {
  ...
  async subscribe() {}
  onMessage(cb: (m: WsMessage) => void) { this.msgCb = cb; }
  onClose(cb: () => void) { this.closeCb = cb; }
  async close() {}
  emit(data: unknown) { this.msgCb?.({ data, received_ms: 0 }); }
  drop() { this.closeCb?.(); }
}
...
describe('WsCollector', () => {
  it('losslessly persists every parsed message', async () => {
    ...
    const col = new WsCollector(tr, store, {
      ...
      now: () => clock,
      parse: (m): RawRecord => ({
        kind: 'trade', symbol: 'KRW-BTC', event_time: iso(m.received_ms), ingest_time: iso(m.received_ms),
        event_time_ms: m.received_ms, ingest_time_ms: m.received_ms, seq: Number(m.data), payload: m.data,
      }),
    });
    await col.start();
    for (let i = 1; i <= 5; i++) { clock = i; tr.emit(i); }
  ...
  it('flags staleness past heartbeat window', async () => {
    ...
    const col = new WsCollector(tr, new InMemoryRawStore(), {
      subscribe: {}, heartbeatMs: 100, now: () => clock, parse: () => null,
    });
    await col.start();
    clock = 50; expect(col.isStale()).toBe(false);
    clock = 200; expect(col.isStale()).toBe(true);
  });
});
