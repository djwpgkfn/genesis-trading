...
const sig = (name: Signal['name'], value: number): Signal => ({
  id: `${name}@1`, name, value, strength: 0.8, confidence: 0.8, timestamp_ms: 1, source: ['f'],
});
...
function checkTC3(): CheckResult {
  ...
  const empty: StrategyDecision = { active: undefined as unknown as StrategyName, selected: [], scores: [] };
  let threw = false;
  try {
  ...
  return threw ? { id: 'INV-TC3', status: 'pass' } : { id: 'INV-TC3', status: 'fail', detail: 'decision made without strategy' };
}
...
function checkTC4(): CheckResult {
  ...
  return threw ? { id: 'INV-TC4', status: 'pass' } : { id: 'INV-TC4', status: 'fail', detail: 'decision made without signals' };
}

/** INV-TC5: Decision confidence is within [0,1]. */
function checkTC5(): CheckResult {
  const d = new DecisionEngine().decide(goodStrategy, [sig('TREND_UP', 1), sig('MACD_BULLISH', 1)], risk, pf, ctx);
  const ok = d.confidence >= 0 && d.confidence <= 1;
  return ok ? { id: 'INV-TC5', status: 'pass' } : { id: 'INV-TC5', status: 'fail', detail: `confidence=${d.confidence}` };
}
...
function checkTC6(): CheckResult {
  ...
  return ok ? { id: 'INV-TC6', status: 'pass' } : { id: 'INV-TC6', status: 'fail', detail: 'missing trace' };
}
