...
export class ControlPlane {
  ...
  deploy(manifest: DeploymentManifest, target: ProductionSnapshot): { ok: boolean; reasons: string[] } {
    ...
    this.emit('Snapshot.activated', target.snapshot_id, { snapshot_id: target.snapshot_id, manifest_id: manifest.manifest_id });
    return { ok: true, reasons: [] };
  }

  /** Champion swap is just a controlled snapshot activation (same signed path). */
  championSwap(manifest: DeploymentManifest, target: ProductionSnapshot): { ok: boolean; reasons: string[] } {
  ...
  rollback(): void {
    ...
    this.emit('Snapshot.rolledBack', active?.snapshot_id ?? 'none', { to: active?.snapshot_id ?? null });
  }

  private emit(type: string, snap: string, payload: unknown): void {
    const input: EventInput = {
      event_id: asUUID(`cp-${type}-${++this.n}`), event_type: type,
      event_time: asISOTimestamp(this.now()), ingest_time: asISOTimestamp(this.now()),
      source_engine: 'control-plane', schema_version: 1,
      correlation_id: asCorrelationId('control-plane'), snapshot_id: asSnapshotId(String(snap)),
      payload,
    };
