...
export const VALID: Record<RuntimeState, readonly RuntimeState[]> = {
  INIT: ['READY'], READY: ['RUN'], RUN: ['SAFE_MODE', 'HALT'], SAFE_MODE: ['RUN', 'HALT'],
  HALT: ['RECOVERY'], RECOVERY: ['READY', 'HALT', 'FROZEN'], FROZEN: [],
};
...
export class ProductionRuntime {
  ...
  transition(to: RuntimeState): void {
    ...
    emitTransition(this.store, { machine: MACHINE, from, to }, {
      event_id: asUUID(`prod-tr-${++this.n}`), at: asISOTimestamp(this.now()),
      correlation_id: asCorrelationId('production'), source_engine: 'production-engine',
    });
  }
  init(): void { this.transition('READY'); }   // INIT → READY (cold start; never direct RUN)
  start(): void { this.transition('RUN'); }    // READY → RUN (explicit)
}
