import { InMemoryEventStore, type EventStore, type EventInput, projectDecision } from '@genesis/event-engine';
...
import { CorrelationMatrix, optimize, type Candidate, type PortfolioConstraints } from '@genesis/portfolio-engine';
...
export interface FeaturePort { compute(): { liquidity: number; volatility: number; trend: number; volume: number } }
export interface StrategyPort { candidates(mh: MarketHealth): Candidate[] }
export interface RiskPort {
  budgetView(): { total: number; available: number };
  preTradeCheck(req: { request_id: string; symbol: string; side: 'buy' | 'sell'; notional: number }): { approved: boolean; token_id?: string; reason: string };
}
...
export class CycleOrchestrator {
  ...
  runCycle(input: CycleInput): CycleOutcome {
    ...
    const emit = (type: string, payload: unknown): void => this.emit(type, correlation_id, input.snapshot_id, payload);
    ...
    emit('Portfolio.planned', { allocations, utilization: allocations.reduce((s, a) => s + a.notional, 0) / (budget.total || 1) });

    // 6) Risk gate per order → 7) Execution gateway (token required)
    const orders: CycleOutcome['orders'] = [];
    for (const a of allocations) {
      const req = { request_id: `${correlation_id}:${a.symbol}`, symbol: a.symbol, side: 'buy' as const, notional: a.notional };
      const decision = this.risk.preTradeCheck(req);
      emit('Risk.decided', { symbol: a.symbol, approved: decision.approved, reason: decision.reason });
      const order: Order = { client_order_id: req.request_id, symbol: a.symbol, side: 'buy', notional: a.notional };
      let executed = false;
  ...
  private emit(type: string, corr: string, snap: string, payload: unknown): void {
    const input: EventInput = {
      event_id: asUUID(`orch-${type}-${corr}`), event_type: type,
      event_time: asISOTimestamp(this.now()), ingest_time: asISOTimestamp(this.now()),
      source_engine: 'cycle-orchestrator', schema_version: 1,
      correlation_id: asCorrelationId(corr), snapshot_id: asSnapshotId(snap), payload,
    };
