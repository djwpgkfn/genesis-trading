...
describe('Research Platform', () => {
  ...
  it('champion/challenger stays Research-internal and requires WFV', () => {
    ...
    const bt = { ...exp('backtest'), status: 'completed' as const, result: { metrics: { activity: 5 }, passed: true, decisions: 6 } };
    expect(cc.evaluate(bt).outcome).toBe('rejected'); // no WFV
    const wfv = { ...exp('wfv'), status: 'completed' as const, result: new ExperimentRunner().runWFV(exp('wfv'), marketLog(), hyp, 3) };
