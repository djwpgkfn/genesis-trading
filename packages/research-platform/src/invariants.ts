...
function checkS5(): CheckResult {
  ...
  const wfvExp = {
    ...exp('wfv'),
    status: 'completed' as const,
    result: runner.runWFV(exp('wfv'), marketLog(), hyp, 3),
  };
...
function checkS6(): CheckResult {
  ...
  const ch1 = {
    ...
    result: {
      ...
      passed: true,
      decisions: 6,
      folds: [{ start: 0, end: 1, passed: true, metrics: {} }],
    },
  };
  const d1 = cc.evaluate(ch1);
  const promotedLifecycleUnchanged =
    d1.outcome === 'promoted-in-research' && ch1.status === 'completed';
  const ch2 = {
    ...exp('wfv'),
    status: 'completed' as const,
    result: {
