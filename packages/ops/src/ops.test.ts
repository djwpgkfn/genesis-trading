...
import { InMemoryEventStore, EventTypes, projectDecision, type EventInput } from '@genesis/event-engine';
...
describe('Health Monitor', () => {
  it('tracks Healthy/Warning/Critical per component', () => {
    ...
    clock += 15_000; expect(hm.status('websocket').status).toBe('warning');
    clock += 20_000; expect(hm.status('websocket').status).toBe('critical');
...
describe('Structured Logging', () => {
  it('includes correlation/request/snapshot/execution/replay/trace ids', () => {
    const out: LogEntry[] = [];
    const log = new StructuredLogger((e) => out.push(e), () => '2026-07-28T00:00:00.000Z', { trace_id: 'T1' });
    log.child({ correlation_id: 'C1', snapshot_id: 'S1' }).info('order', { request_id: 'R1', execution_id: 'X1', replay_id: 'RP1' });
    const ctx = out[0]!.ctx;
    expect(ctx).toMatchObject({ trace_id: 'T1', correlation_id: 'C1', snapshot_id: 'S1', request_id: 'R1', execution_id: 'X1', replay_id: 'RP1' });
  });
});
...
describe('Metrics', () => {
  it('records counters, histograms, and timers', () => {
    ...
    const stop = m.timer(METRICS.cycleTimeMs); clock = 42; stop();
...
describe('Failure Automation (determinism preserved)', () => {
  it('auto-creates replay session + incident report linked to DecisionRecords', () => {
    ...
    const logger = new StructuredLogger(() => {}, () => '2026-07-28T00:00:00.000Z');
    const fa = new FailureAutomation(src, new InMemoryEventStore(), alerts, logger, () => '2026-07-28T00:00:00.000Z');
  ...
  it('ops does NOT change decisions (Replay determinism preserved)', () => {
    ...
    const fa = new FailureAutomation(src, new InMemoryEventStore(), new AlertSystem(), new StructuredLogger(() => {}));
