...
const sig = (name: SignalName, value: number, strength = 0.8, confidence = 0.8): Signal => ({
  id: `${name}@1`, name, value, strength, confidence, timestamp_ms: 1, source: ['f'],
});

describe('StrategyEngine', () => {
  it('selects the highest-scoring strategy as active', () => {
    const d = new StrategyEngine().select([sig('TREND_UP', 1), sig('EMA_CROSS', 1), sig('MACD_BULLISH', 1)]);
  ...
  it('scores are sorted descending and confidence in [0,1]', () => {
    ...
    for (let i = 1; i < d.scores.length; i++) expect(d.scores[i - 1]!.score).toBeGreaterThanOrEqual(d.scores[i]!.score);
