...
function checkTC2(): CheckResult {
  ...
  const ok =
    withSignals.selected.length >= 1 &&
    !!withSignals.active &&
    noSignals.selected.length >= 1 &&
    !!noSignals.active;
  return ok
    ? { id: 'INV-TC2', status: 'pass' }
    : { id: 'INV-TC2', status: 'fail', detail: 'no strategy selected' };
}

export const strategyChecks: ReadonlyArray<{ id: string; fn: () => CheckResult }> = [
  { id: 'INV-TC2', fn: checkTC2 },
];
