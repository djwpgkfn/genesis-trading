...
export interface CalibrationPoint { confidence: number; realized: number } // 0..1
...
export class AIMemory {
  ...
  calibrationError(): number {
    ...
    return this.state.calibration.reduce((a, c) => a + Math.abs(c.confidence - c.realized), 0) / this.state.calibration.length;
  }
  ...
  drift(): number {
    ...
    const err = (arr: CalibrationPoint[]) => arr.reduce((a, x) => a + Math.abs(x.confidence - x.realized), 0) / (arr.length || 1);
