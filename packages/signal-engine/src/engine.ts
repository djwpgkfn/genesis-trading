...
export class SignalEngine {
  generate(snap: MarketSnapshot): SignalSet {
    ...
    if (eShort !== null && eLong !== null) {
      ...
      out.push(
        signal('EMA_CROSS', Math.sign(diff), strength, clamp01(0.5 + strength), t, [
          `ema9=${eShort.toFixed(2)}`,
          `ema21=${eLong.toFixed(2)}`,
        ]),
      );
    }
    ...
    if (m !== null) {
      ...
      if (m.hist >= 0)
        out.push(
          signal('MACD_BULLISH', 1, strength, clamp01(0.55 + strength), t, [
            `macd=${m.macd.toFixed(3)}`,
            `signal=${m.signal.toFixed(3)}`,
          ]),
        );
      else
        out.push(
          signal('MACD_BEARISH', -1, strength, clamp01(0.55 + strength), t, [
            `macd=${m.macd.toFixed(3)}`,
            `signal=${m.signal.toFixed(3)}`,
          ]),
        );
    ...
    if (r !== null) {
      if (r < 30)
        out.push(
          signal('RSI_OVERSOLD', 1, clamp01((30 - r) / 30), clamp01(0.6 + (30 - r) / 60), t, [
            `rsi=${r.toFixed(1)}`,
          ]),
        );
      else if (r > 70)
        out.push(
          signal('RSI_OVERBOUGHT', -1, clamp01((r - 70) / 30), clamp01(0.6 + (r - 70) / 60), t, [
            `rsi=${r.toFixed(1)}`,
          ]),
        );
    }
    ...
    if (bb !== null) {
      if (last > bb.upper)
        out.push(
          signal('BB_BREAKOUT', 1, clamp01((last - bb.upper) / (last || 1)), 0.6, t, [
        ...
        out.push(
          signal('BB_BREAKOUT', -1, clamp01((bb.lower - last) / (last || 1)), 0.6, t, [
            `close=${last.toFixed(2)}`,
    ...
    if (vr !== null) {
      if (vr > 1.5)
        out.push(
          signal('HIGH_VOLUME', 1, clamp01((vr - 1) / 2), clamp01(0.5 + (vr - 1.5) / 3), t, [
            `vol_ratio=${vr.toFixed(2)}`,
          ]),
        );
      else if (vr < 0.5)
        out.push(signal('LOW_VOLUME', -1, clamp01(1 - vr), 0.5, t, [`vol_ratio=${vr.toFixed(2)}`]));
    }
    ...
    if (vol !== null && vol > 0.02)
      out.push(
        signal('VOLATILITY_HIGH', 1, clamp01(vol * 10), clamp01(0.5 + vol * 5), t, [
    ...
    if (slope !== null) {
      ...
      if (slope > 0)
        out.push(
          signal('TREND_UP', 1, strength, clamp01(0.5 + strength), t, [
            `slope=${slope.toFixed(4)}`,
          ]),
        );
      else if (slope < 0)
        out.push(
          signal('TREND_DOWN', -1, strength, clamp01(0.5 + strength), t, [
            `slope=${slope.toFixed(4)}`,
          ]),
        );
    }
    ...
    if (snap.orderbook) {
      ...
      if (total > 0) {
        ...
        if (Math.abs(imb) > 0.1)
          out.push(
            signal(
              'ORDERBOOK_IMBALANCE',
              Math.sign(imb),
              clamp01(Math.abs(imb)),
              clamp01(0.5 + Math.abs(imb) / 2),
              t,
              [`bid=${bid}`, `ask=${ask}`],
            ),
          );
        if (total < 1)
          out.push(
            signal('LIQUIDITY_LOW', -1, clamp01(1 - total), 0.5, t, [`depth=${total.toFixed(3)}`]),
          );
        else if (total > 100)
          out.push(
            signal('LIQUIDITY_HIGH', 1, clamp01(total / 1000), 0.5, t, [
              `depth=${total.toFixed(1)}`,
            ]),
          );
      }
    }
