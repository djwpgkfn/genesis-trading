...
function sampleSnapshot(): MarketSnapshot {
  ...
  return {
    symbol: 'KRW-BTC',
    timestamp_ms: 40 * 60000,
    candles,
    orderbook: { bids: [{ price: 99, size: 60 }], asks: [{ price: 101, size: 40 }] },
  };
}
...
function checkTC1(): CheckResult {
  ...
  return ok
    ? { id: 'INV-TC1', status: 'pass' }
    : { id: 'INV-TC1', status: 'fail', detail: 'signal confidence/strength/source invalid' };
}

export const signalChecks: ReadonlyArray<{ id: string; fn: () => CheckResult }> = [
  { id: 'INV-TC1', fn: checkTC1 },
];
